<?php
/**
 * Cliente para la API de Voluum (Versión Final con Reporte Correcto)
 */
class VoluumAPI {
    private $accessKeyId;
    ...
    public function __construct() {
        ...
        $this->apiUrl = VOLUUM_CONFIG['api_url']; // Debe ser https://panel-api2.voluum.com
    ...
    public function makeRequest($fullUrl) {
        if ($this->sessionToken === null) {
            $this->getSessionToken();
        }
        $headers = ["Accept: application/json", "cwauth-token: {$this->sessionToken}"];
        $ch = curl_init();
        curl_setopt_array($ch, [CURLOPT_URL => $fullUrl, CURLOPT_RETURNTRANSFER => true, CURLOPT_HTTPGET => true, CURLOPT_HTTPHEADER => $headers]);
        $response = curl_exec($ch); $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE); curl_close($ch);
        $responseData = json_decode($response, true);
        if ($httpCode >= 400) {
    ...
    // Este método ahora construye la URL que sabemos que funciona
    public function getReportByAdForDate($voluumCampaignId, $from, $to, $currency = 'USD') {
        ...
        // El bloque que faltaba: la definición del array $params
        $params = [
            'from' => $from,
            'to' => $to,
            'tz' => 'Etc/GMT',
        ...
        // Añadimos las columnas que nos interesan
